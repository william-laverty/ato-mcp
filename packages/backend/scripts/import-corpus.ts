 ...
 *   pnpm tsx packages/backend/scripts/import-corpus.ts \
 *     [--sqlite ~/Library/Application\ Support/ato-pro/live/ato.sqlite] \
 *     [--skip docs|chunks|anchors|definitions|thresholds] \
 *     [--batch 500]
 *
 * Idempotent: every insert uses upsert on the primary key.
 * Resumable: failed batches log + script continues; rerun safely.
 */
...
function isoDateOrNull(value: string | null | undefined): string | null {
  ...
  // Supabase DATE column accepts ISO date or full timestamp; truncate if needed
  return value.slice(0, 10);
}

function bytesToPgVector(buf: Buffer | Uint8Array | null): string | null {
  if (!buf || buf.byteLength === 0) return null;
  // sqlite-vec stores embeddings as 384 little-endian float32s = 1536 bytes
...
async function importChunks() {
  ...
  // Stream-read in pages to keep memory bounded
...
async function importAnchors() {
  ...
  // Filter rows that reference chunks present in our chunks insert.
  // (Anchors that point at chunks excluded by some future filter would orphan.)
  // For a full import we assume parity; just upsert.
...
async function importDefinitions() {
  ...
  // No natural unique key — re-import would dupe. Truncate first if rerunning.
  // For simplicity, skip duplicates by inserting and ignoring conflicts.
  // The definitions Postgres table has a synthetic BIGSERIAL primary key, so
  // we can't dedupe via upsert. Use plain insert — caller should truncate
  // definitions before rerunning if needed.
