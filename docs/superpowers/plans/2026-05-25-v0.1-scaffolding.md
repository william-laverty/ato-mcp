# ato-mcp v0.1 (scaffolding) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a working `ato-mcp` Node.js MCP server with parity to the reference `ato-mcp`'s core retrieval surface — `search`, `get_chunks`, `fetch`, `stats` — backed by a locally-built SQLite corpus of ATO website content embedded with all-MiniLM-L6-v2 (384-dim).
...
name = "ato-pipeline"
version = "0.1.0"
description = "ato-mcp corpus pipeline"
...
- Create: `packages/mcp/bin/ato-mcp.js`
...
```bash
pnpm --filter @ato-mcp/mcp test embed/
```
...
/** Used by the `ato-mcp stats` CLI command. */
...
```bash
pnpm --filter @ato-mcp/mcp test tools/stats
```
...
```ts
import type { SearchInput, SearchHit } from "@ato-mcp/shared";
import type { Store } from "../store/types.js";
...
```bash
pnpm --filter @ato-mcp/mcp test tools/search
```
...
```ts
import type { GetChunksInput, SearchHit } from "@ato-mcp/shared";
import type { Store } from "../store/types.js";
...
```bash
pnpm --filter @ato-mcp/mcp test tools/get_chunks
```
...
```ts
import type { FetchInput } from "@ato-mcp/shared";
...
async function fetchHttp(uri: string, url: string): Promise<FetchOutput> {
  const resp = await fetch(url, { headers: { "user-agent": "ato-mcp/0.1" } });
...
describe("MCP stdio protocol", () => {
  beforeAll(async () => {
    ...
    proc = spawn(process.execPath, [path.join(__dirname, "..", "..", "src", "index.ts.compiled.js")], {
      env: { ...process.env, ATO_MCP_DATA_DIR: tmpDataDir },
    });
  ...
  it("responds to initialize", async () => {
    ...
    expect(out.result?.serverInfo?.name).toBe("ato-mcp");
  });
...
```bash
pnpm --filter @ato-mcp/mcp test test/e2e
```
...
export async function runMcp(): Promise<void> {
  ...
  const server = new Server(
    { name: "ato-mcp", version: "0.1.0" },
    {
      capabilities: { tools: {} },
      instructions:
        "ato-mcp provides retrieval over the Australian Taxation Office corpus. Call `stats` first to verify the corpus is installed. If `installed:false`, tell the user to run `ato-mcp update`. Use `search` for hybrid retrieval; resolve `[doc:X]` citations via `get_chunks`. Use `fetch` only when the corpus does not contain the page.",
    },
  );
...
## Task 15: `ato-mcp update` — corpus install/refresh
...
```bash
pnpm --filter @ato-mcp/mcp test test/lib/download
```
